...
program.command("env").option("--addr <addr>").option("--token <token>").option("--github-token <token>").option("-o, --output <path>", ".env").option("--verbose", "Enable verbose output for debugging").action(async (opts) => {
  ...
  try {
    ...
    const templatePath = import_path.default.resolve(process.cwd(), templateName);
...
Expected file like: .env.${env}.tpl in project root`
    ...
    const authMethod = opts.token || process.env.VAULT_TOKEN ? "VAULT_TOKEN" : "GITHUB_TOKEN";
    log.info("Auth", authMethod);
    if (verbose) {
      log.verbose(`Vault address: ${addr}`);
    }
    const outputPath = opts.output ?? ".env";
    const resolvedOutputPath = import_path.default.resolve(process.cwd(), outputPath);
    const dir = import_path.default.dirname(resolvedOutputPath);
    ...
    const relativePath = import_path.default.relative(process.cwd(), resolvedOutputPath);
